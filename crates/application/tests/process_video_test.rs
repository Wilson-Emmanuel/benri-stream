...
use domain::ports::transcoder::{MockTranscoderPort, ProbeResult, TranscoderError};
...
#[tokio::test]
async fn happy_path_probes_transcodes_marks_processed_and_cleans_original() {
    let id = VideoId::new();
    let id_c = id.clone();
...
async fn transcode_failure_transitions_to_failed_and_schedules_delete() {
    ...
    transcoder
        .expect_transcode_to_hls()
        .returning(|_, _, _| Err(TranscoderError::TranscodeFailed("pipeline".into())));
