...
/// Task repository for worker-internal lifecycle operations. Single-task
/// creation from use cases goes through `TaskScheduler` + `TaskMutations`
/// inside a `TxScope` — see `crate::ports::unit_of_work`. Bulk creation
/// (e.g. from the cleanup sweep) is pool-backed since it's a single
/// statement and has no business mutation to bundle with.
#[async_trait]
