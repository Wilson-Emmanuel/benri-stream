...
| **S3 Storage** | Uploaded originals (temp) and HLS output (permanent). Presigned URLs for upload. CDN origin |
...
8. Frontend polls `GET /api/videos/{id}/status` until the link appears, then redirects to the player
...
- **Kafka/SQS/RabbitMQ** — production-grade but heavy for two task types

Redis works because it's already needed for the distributed lock. If it loses data, the
...
| Approach | Time-to-first-segment | Total time | CPU usage |
|----------|----------------------|------------|-----------|
| **Parallel** (chosen) | ~4s | Shortest — one decode pass | High — 3 encoders |
| Sequential per level | ~4s | ~3× longer — 3 decode passes | Low — 1 encoder |
| Low first, then parallel | ~4s | ~1.5× longer — 2 decode passes | Varies |

Parallel wins on total processing time because it decodes the input once instead of
three times. The tradeoff is higher peak CPU, but workers scale horizontally.

### 4-second HLS segments

| Duration | Time-to-stream | File count | CDN efficiency |
|----------|---------------|------------|----------------|
| 2s | ~2s | High | More cache misses |
| **4s** | **~4s** | **Moderate** | **Good** |
| 6–10s | ~6–10s | Low | Best |

4 seconds balances fast time-to-stream with manageable file count and good CDN cache
behavior.
...
and cost scales linearly. CDN flips this: origin hit once per segment, edge nodes serve
...
| Codec | Browser support | HLS compatible | License |
|-------|----------------|----------------|---------|
| **H.264** (chosen) | Universal | Yes | Free for streaming |
| H.265/HEVC | Partial | Poor real-world compat | Heavy patents |
| VP9 | No Safari HLS | No | Royalty-free |
| AV1 | Newer browsers only | No | Royalty-free |

H.264 is the only codec that works everywhere with HLS.

### Other decisions

- **Three quality levels (360p, 720p, 1080p)** — covers mobile to desktop. Two misses high-quality desktop, four+ adds marginal gain for more storage and encoding time.
...
| CDN | Nginx (dev) / Cloudflare | CloudFront, Fastly |
...
- **Early-publish playback (Safari/Chrome)** — During transcoding, the worker publishes
  a growing HLS EVENT playlist. Firefox (via hls.js) handles this correctly: plays
  ...
  Both work correctly once all tiers finish and `#EXT-X-ENDLIST` is written.
...
| CDN | Nginx (dev) / Cloudflare (prod) | `nginx:alpine` (cache proxy) |
